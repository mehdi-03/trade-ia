...
async def init_db():
    ...
    try:
        ...
        engine = create_async_engine(
            ...
            poolclass=NullPool,  # Pour éviter les problèmes de connexion
        )
        ...
        async with tsdb_engine.begin() as conn:
            await conn.run_sync(Base.metadata.create_all)
            
            # Création des hypertables TimescaleDB
            await conn.execute(
                "SELECT create_hypertable('market_data', 'timestamp', if_not_exists => TRUE);"
            )
            await conn.execute(
                "SELECT create_hypertable('crypto_data', 'timestamp', if_not_exists => TRUE);"
            )
            await conn.execute(
