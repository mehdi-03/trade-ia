...
class MessageQueue:
    ...
    async def declare_queue(
        ...
        """Déclare une queue avec ses bindings."""
        if not self.channel:
            await self.connect()
    ...
    async def publish_batch(
        self,
        exchange: str,
        messages: list[tuple[str, Dict[str, Any]]]  # [(routing_key, message), ...]
    ):
        ...
        logger.info(f"Batch publié: {len(messages)} messages", exchange=exchange)
